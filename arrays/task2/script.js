...
const verifyCats = function (arr1, arr2) {
  // 1
  const catsJaneNew = arr1.slice(1, -1);

  // 2
  const catsJaneAndJulia = [...catsJaneNew, ...arr2];
  console.log(catsJaneAndJulia);


  // 3
  catsJaneAndJulia.forEach(function (age, catsSerialNumber) {
    if (age > 2) {
      console.log(`Кошка №${catsSerialNumber + 1} взрослая, ей ${age} лет`);
    } else {
      console.log(`Кошка №${catsSerialNumber + 1} ещо котенок`);
    }
